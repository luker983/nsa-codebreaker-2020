...
def run(host, port, v=False):
    ...
    # make peers request with message type == 1
    frame = make_frame(make_command(terminal_type, my_name, 0, 1))
    ...
    # parse out all peer names
    (flags, msg, unknown_zeros), rest = parse_pkt(pkt)
    while len(rest) > 0:
        node = rest[:35]
        rest = rest[35:]
        unknown_maybe_random, nodetype, name = struct.unpack('>HB32s', node)
        name = name.rstrip(b'\x00')
        print('PEER:', 'unknown:', hex(unknown_maybe_random), 'type:', nodetype, 'name:', name)

# send things other than HELLO messages
def make_command(nodetype, name, flag_type, msg_type):
